...
function Footer({ audio, currentTime, footeraudioState, audiostate, playing, dispatch }) {
  ...
  const playSong = () => {
    dispatch({
      ...
      payload: {
        playSong: true,
        playinglist: footeraudioState.url,
      }
    });
  };
  ...
  const getFilterList = () => {
    if (footeraudioState[0].track) {
      let filterUrl = footeraudioState.audiolist.filter(
  ...
  const getCurrentIndex = () => {
    if (footeraudioState.audiolist[0].track) {
      let currentIndex = footeraudioState.audiolist
        .filter((item) => item.track?.preview_url !== null)
        .map((item, index) => {
          if (
            item.track?.preview_url === footeraudioState.url
          ) {
            return index;
          } else {
            return undefined;
      ...
      let currentIndex = footeraudioState.audiolist
        ...
        .map((item, index) => {
          if (item.preview_url === footeraudioState.url) {
            return index;
          } else {
            return undefined;
          }
    ...
    <FooterContainer>
      ...
      <FooterCenter>
        <IconContainer>
          ...
          />
          {audiostate === footeraudioState?.url &&
          playing ? (
...
export default connect(({global}) => ({...global}))(Footer);
